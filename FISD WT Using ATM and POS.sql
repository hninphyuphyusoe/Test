...
SELECT '20240920' AS EDate,
	...
	CATEGORY_OF_CARD, CURRENCY, AUTHTXN_CARD_NO AS Used_Card_Quantity, SOURCE, USED_LOCATION, TRANS_COUNT,  
		     ...
		     CASE WHEN (authTXN_CURRENCY_CODE in (156,978,392 ,458,702,764) and authTXN_CURRENCY_CODE != authTXN_BILLING_CURRENCY_ID)
              THEN (authTXN_BILLING_TXN_AMT/ (select rate from USD_rate where C_code = authTXN_CURRENCY_CODE))
              ...
              THEN (authTXN_BILLING_TXN_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'VISA'))
              ...
              THEN (authTXN_BILLING_TXN_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'JCB'))
              ...
              THEN (AUTHTXN_BILLING_TXN_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'MASTER'))
              WHEN (authTXN_CURRENCY_CODE not in (104,156,978,392 ,458,702,764) and  authTXN_CURRENCY_CODE != authTXN_BILLING_CURRENCY_ID)
              THEN (authTXN_BILLING_TXN_AMT/ (select rate from USD_rate where C_code = 840))
              ELSE authTXN_BILLING_TXN_AMT END  AS Trans_Amt,

      CASE WHEN authTXN_BILLING_CURRENCY_ID != 840 AND authTXN_BILLING_CURRENCY_ID = '104'  AND Card_Name LIKE '%JCB'
              THEN (authTXN_APPROVED_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'JCB'))
              WHEN authTXN_BILLING_CURRENCY_ID != 840 AND authTXN_BILLING_CURRENCY_ID = '104'  AND Card_Name = 'Visa'
              THEN (authTXN_APPROVED_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'VISA'))
              WHEN authTXN_BILLING_CURRENCY_ID != 840 AND authTXN_BILLING_CURRENCY_ID = '104'  AND Card_Name LIKE '%Master'
              THEN (authTXN_APPROVED_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'MASTER'))
              WHEN authTXN_BILLING_CURRENCY_ID != 840 AND authTXN_BILLING_CURRENCY_ID in (104,156,978,392,458,702,764) 
              THEN (authTXN_BILLING_TXN_AMT/ (select rate from USD_rate where C_code = 840))
              ELSE authTXN_BILLING_TXN_AMT END  AS  Trans_Amt_USD,
         ...
         CASE WHEN authTXN_CURRENCY_CODE != '104' AND authTXN_CURRENCY_CODE = authTXN_BILLING_CURRENCY_ID AND Card_Name LIKE '%JCB'
              ...
              THEN authTXN_BILLING_TXN_AMT*(select rate from USD_rate where C_code = authTXN_CURRENCY_CODE )
              ELSE authTXN_BILLING_TXN_AMT end AS Trans_Amt_MMK,        
...
SELECT * FROM (
		SELECT AUTHTXN_RETRIEVAL_REFNO AS REF_NO,AUTHTXN_CRDACCT_NO AS CARD_NO, AUTHTXN_CRDPLAN_ID,AUTHTXN_TXNTYPE_ID,AUTHTXN_CUST_ID,
			...
			CASE WHEN TXNTYPE_BASE_TXNTYPE_ID IN (''PAYMENT'',''TOPUP'') THEN 0 ELSE 1 END AS TRANS_COUNT,
			     ...
			     AUTHTXN_CARD_NO, AUTHTXN_CURRENCY_CODE, AUTHTXN_BILLING_CURRENCY_ID, AUTHTXN_BILLING_TXN_AMT, AUTHTXN_APPROVED_AMT,   
...
SELECT 
	...
	CATEGORY_OF_CARD, CURRENCY, OAUTHTXN_CARD_NO AS Used_Card_Quantity, SOURCE, USED_LOCATION, TRANS_COUNT, 
	    ...
	    CASE WHEN (OauthTXN_CURRENCY_CODE in (156,978,392 ,458,702,764) and OauthTXN_CURRENCY_CODE != OAUTHTXN_BILLING_CURRENCY_ID)
              THEN (OauthTXN_BILLING_TXN_AMT/ (select rate from USD_rate where C_code = OauthTXN_CURRENCY_CODE))
              ...
              THEN (OauthTXN_BILLING_TXN_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'VISA'))
              ...
              THEN (OauthTXN_BILLING_TXN_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'JCB'))
              ...
              THEN (OAUTHTXN_BILLING_TXN_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'MASTER'))
              WHEN (OauthTXN_CURRENCY_CODE not in (104,156,978,392 ,458,702,764) and  OauthTXN_CURRENCY_CODE != OAUTHTXN_BILLING_CURRENCY_ID)
              THEN (OauthTXN_BILLING_TXN_AMT/ (select rate from USD_rate where C_code = 840))
              ELSE OauthTXN_BILLING_TXN_AMT END  AS Trans_Amt,
       
         CASE WHEN OAUTHTXN_BILLING_CURRENCY_ID != 840 AND OAUTHTXN_BILLING_CURRENCY_ID = '104'  AND Card_Name LIKE '%JCB'
              THEN (OauthTXN_APPROVED_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'JCB'))
              WHEN OAUTHTXN_BILLING_CURRENCY_ID != 840 AND OAUTHTXN_BILLING_CURRENCY_ID = '104'  AND Card_Name = 'Visa'
              THEN (OauthTXN_APPROVED_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'VISA'))
              WHEN OAUTHTXN_BILLING_CURRENCY_ID != 840 AND OAUTHTXN_BILLING_CURRENCY_ID = '104'  AND Card_Name LIKE '%Master'
              THEN (OauthTXN_APPROVED_AMT/ (select RATE from CBM_Exchange_Rate where CurrencyCode = 840 AND CardBrand = 'MASTER'))
              WHEN OAUTHTXN_BILLING_CURRENCY_ID != 840 AND OAUTHTXN_BILLING_CURRENCY_ID in (104,156,978,392,458,702,764) 
              THEN (OauthTXN_BILLING_TXN_AMT/ (select rate from USD_rate where C_code = 840))
              ELSE OauthTXN_BILLING_TXN_AMT END  AS  Trans_Amt_USD,
         ...
         CASE WHEN OauthTXN_CURRENCY_CODE != '104' AND OauthTXN_CURRENCY_CODE = OauthTXN_BILLING_CURRENCY_ID AND Card_Name LIKE '%JCB'
              ...
              THEN OauthTXN_BILLING_TXN_AMT*(select rate from USD_rate where C_code = OauthTXN_CURRENCY_CODE )
              ELSE OauthTXN_BILLING_TXN_AMT end AS Trans_Amt_MMK, 
...
SELECT * FROM (
	...
	-- (CASE WHEN(OAUTHTXN_GEOGRAPHY_IND <> ''IM'' and authTXN_MCC_ID in (''6011'',''6010'') and OAUTHTXN_SOURCE in (''MPU'',''CR2''))
									-- OR (OAUTHTXN_GEOGRAPHY_IND = ''IM'' and authTXN_MCC_ID in (''6011'',''6010'') and OAUTHTXN_SOURCE in (''JCB'',''CUP'',''VAP'')) THEN ''ATM'' ELSE 
                   -- CASE WHEN authTXN_TXN_ENTRY_MODE LIKE ''%ECOM%'' OR authTXN_TXN_ENTRY_MODE =''RECURRING'' 
                                    -- OR authTXN_TXN_ENTRY_MODE = ''MOTO''   OR authTXN_TXN_ENTRY_MODE is null THEN ''E-Commerce'' ELSE 
                   -- CASE WHEN TXNTYPE_BASE_TXNTYPE_ID IN (''PAYMENT'',''TOPUP'') THEN ''NONE'' ELSE ''POS'' END END END) AS SOURCE,
         ...
         ELSE OAUTHTXN_MERCHANT_ID END AS TRAN_CATEGORY,
			...
			CASE WHEN TXNTYPE_BASE_TXNTYPE_ID IN (''PAYMENT'',''TOPUP'') THEN 0 ELSE 1 END AS TRANS_COUNT,
			   ...
			   OAUTHTXN_CARD_NO, OAUTHTXN_CURRENCY_CODE, OAUTHTXN_BILLING_CURRENCY_ID, OAUTHTXN_BILLING_TXN_AMT, OAUTHTXN_APPROVED_AMT,      

			CASE WHEN OAUTHTXN_TXNTYPE_ID IN (''REFUND'',''RFNDNOAUTH'',''REFUND_PRE'',''REFUND1'',''REFUND_PRV'',''REFUNDADV'') THEN 1 ELSE 0 END AS TRAN_TYPE,
...
ALL_CZ_DATA AS (
					SELECT DISTINCT * FROM (
						SELECT 
							...
							AUTHTXN_DEST,AUTHTXN_CUST_ID,AUTHTXN_MERC_MDR_AMT,AUTHTXN_APPROVED_AMT,AUTHTXN_BILLING_TXN_AMT,AUTHTXN_REQUEST_DATE,
 ...
 SELECT AUTHTXN_RETRIEVAL_REFNO,
       ...
       CASE WHEN CURRENCY_CODE NOT IN (''MMK'',''USD'',''EUR'',''SGD'',''THB'',''CNY'',''MYR'',''JPY'') THEN ''USD''
           ...
           CASE WHEN TXNTYPE_BASE_TXNTYPE_ID IN (''PAYMENT'',''TOPUP'') THEN 0 ELSE 1 END AS TRANS_COUNT,                                 
				   ...
				   AUTHTXN_CUST_ID,
				 CASE WHEN AUTHTXN_TXNTYPE_ID IN (''REFUNDADV'',''RFNDNOAUTH'',''REFUND'',''RRFNDNOAUT'') THEN 1 ELSE 0 END AS TRAN_TYPE,
				  AUTHTXN_DEST,
				  AUTHTXN_SOURCE,
...
ALL_CZ_DATA AS (
					SELECT DISTINCT * FROM (
						SELECT 
							...
							OAUTHTXN_BILLING_TXN_AMT AS AUTHTXN_BILLING_TXN_AMT,OAUTHTXN_REQUEST_DATE AS AUTHTXN_REQUEST_DATE,
						...
						WHERE OAUTHTXN_REQUEST_DATE =''20250520''
						AND (TXNTYPE_REV_VOID_IND IS NULL OR (TXNTYPE_REV_VOID_IND != ''R'' AND TXNTYPE_REV_VOID_IND != ''V''))
						AND OAUTHTXN_TXNTYPE_ID NOT IN (''ACCTVER'',''ACCVER_QC'',''ATRFER'',''BALINQ'',''BALINQ_CA'',''BALINQ_CU'',''BALINQ_SA'',''BALINQ_UNI'',''PINCHG'',''VPREAUTH'',''RSALENOAUT'')
						AND OAUTHTXN_RESPONSE_CODE=''00''
						AND OAUTHTXN_MCC_ID != ''6011''
...
ALL_CZ_DATA AS (
					SELECT DISTINCT * FROM (
						SELECT 
							...
							AUTHTXN_DEST,AUTHTXN_CUST_ID,AUTHTXN_MERC_MDR_AMT,AUTHTXN_APPROVED_AMT,AUTHTXN_BILLING_TXN_AMT,
 ...
 SELECT AUTHTXN_RETRIEVAL_REFNO,
	   ...
	   CASE WHEN AUTHTXN_MCC_ID= ''6011''  THEN ''ATM''
		...
		1 AS USD_RATE,
		--CASE WHEN AUTHTXN_TXNTYPE_ID IN (''REFUNDADV'',''RFNDNOAUTH'',''REFUND'',''RRFNDNOAUT'') THEN 1 ELSE 0 END AS TRAN_TYPE,
		CASE WHEN AUTHTXN_TXNTYPE_ID IN (''REFUND'', ''REFUND_PRE'',''REFUND_PRV'', ''REFUND1'', ''REFUNDADV'', ''RFNDNOAUTH'', ''RREFUND'', ''RRFNDNOAUT'') 
		THEN 1 ELSE 0 END AS TRAN_TYPE,
		AUTHTXN_DEST,
		AUTHTXN_SOURCE,
		AUTHTXN_GEOGRAPHY_IND,
...
ALL_CZ_DATA AS (
					SELECT DISTINCT * FROM (
						SELECT 
							...
							OAUTHTXN_MERC_MDR_AMT AS AUTHTXN_MERC_MDR_AMT,
							OAUTHTXN_APPROVED_AMT AS AUTHTXN_APPROVED_AMT,
							OAUTHTXN_BILLING_TXN_AMT AS AUTHTXN_BILLING_TXN_AMT,OAUTHTXN_SETTLED_DATE as AUTHTXN_SETTLED_DATE,
							OAUTHTXN_BILLING_CURRENCY_ID AS AUTHTXN_BILLING_CURRENCY_ID,
							TXNTYPE_BASE_TXNTYPE_ID,BILLCURR.CURRENCY_CODE,CARDTYPE.INFO_DESC,CRDBRAND_TYPE,CRDPLAN_PRODUCT_NAME,
...
---- WHERE P37_RRN NOT IN (SELECT AUTHTXN_RETRIEVAL_REFNO FROM OPENQUERY(MSTOCZ,
--        -- ' SELECT AUTHTXN_RETRIEVAL_REFNO FROM CZ_AUTHTXN  WHERE TO_CHAR(TO_DATE(authTXN_REQUEST_DATE, ''yyyymmdd''),''YYYYMMDD'') = TO_CHAR(sysdate-1,''YYYYMMDD'')'
--    -- )) --- END >>  CR2 : ATM 
